...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        elif m is MobileNetv4:
            c2 = args[0]
            args = args[1:]  # 把yaml文件的args[1]作为传入模块的参数
        else:
            c2 = ch[f]

        #m_ = nn.Sequential(*(m(*args[:len(m.__init__.__code__.co_varnames) - 1]) for _ in range(n))) if n > 1 else m(*args[:len(m.__init__.__code__.co_varnames) - 1])
