...
MNV4ConvSmall_Block_Specs = {
    "conv0": {
        ...
        "num_blocks": 1,
        "block_specs": [
            [3, 32, 3, 2] # inchannel outchannel kernel stride
        ],
    },
    "layer1": {
        "block_name": "convbn",
...
def conv2d(in_channels, out_channels, kernel_size=3, stride=1, groups=1, bias=False, norm=True, act=True):
    # 使用 nn.Sequential 的构造函数来添加模块
    layers = []
    padding = (kernel_size - 1) // 2
    layers.append(nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=bias, groups=groups))
...
def build_blocks(layer_spec): # 解析配置文件的参数 layer_spec = {"block_name": "convbn","num_blocks": 1,"block_specs": [[3, 32, 3, 2]]}
    global msha # 全局变量
    if not layer_spec.get("block_name"): # 如果无法获得'block_name'的值
        return nn.Sequential()    # 返回空
    block_names = layer_spec["block_name"] # block_names = "convbn"
    layers = nn.Sequential()
    if block_names == "convbn":
        schema_ = ["in_channels", "out_channels", "kernel_size", "stride"] #
        for i in range(layer_spec["num_blocks"]): #for i in range(1)
            args = dict(zip(schema_, layer_spec["block_specs"][i])) #  把zip(schema_,[3,32,3,2])转化为字典
            layers.add_module(f"convbn_{i}", conv2d(**args)) # 添加模型
...
class MobileNetv4(nn.Module):
    def __init__(self, slice,model = 'MNV4ConvSmall', num_classes=1): # model = 'MNV4ConvSmall'
        ...
        assert model in MODEL_SPECS.keys() # model = "MNV4ConvSmall"
        self.model = model
        self.num_classes = num_classes
        self.spec = MODEL_SPECS[self.model] # spec = 'MNV4ConvSmall_Block_Specs'

        # conv0
        self.conv0 = build_blocks(self.spec["conv0"])  # conv0 = 2维卷积的模型
        # layer1
        ...
        if slice == 1:
            self.model =nn.Sequential(self.conv0, self.layer1, self.layer2)
        elif slice == 2:
            self.model =nn.Sequential(self.layer3)
        elif slice == 3:
            self.model =nn.Sequential(self.layer4)
